...
|ALDS1_10_B|Matrix Chain Multiplication       |連鎖行列積問題<br>(行列の乗算回数を最小にする計算順を求める)|
...
<!--
- CGL_1_A  Projection   射影  
    直線(p1,p2)に対して点pから引いた垂線とその直線の交点の座標を求める。  

- CGL_1_B  Reflection  反射  
    点pから直線(p1,p2)に対して線対称の位置にある座標を求める。  
...
- CGL_2_A  Parallel/Orthogonal  直線の直交・平行判定  
    直線(p0,p1)と直線(p2,p3)が、1:直交か、2:平行か、3:そのどちらでもないか判定する。  

- CGL_2_B  Intersection  線分の交差判定  
    線分s1(p0,p1)、線分s2(p2,p3)が交差するか判定する。     

- CGL_2_C  CrossPoint  線分の交点  
    線分s1(p0,p1)、線分s2(p2,p3)の交点を求める。  

- CGL_2_D  Distance  線分と線分の距離  
    線分s1(p0,p1)、線分s2(p2,p3)の間の距離を求める。  

- CGL_3_A  Area  
    多角形g(凸包とは限らない)の面積を求める。  
    各頂点座標(x<sub>i</sub>,y<sub>i</sub>)は反時計回りに与えられる。  

- CGL_3_B  Is-Convex  
    多角形gが凸包かどうかを判定する。  

- CGL_3_C  Polygon-PointContainment  点の内包  
    点pが多角形g(凸包とは限らない)の内部にあるかどうか判定する。  

- CGL_4_A  ConvexHull   凸包  
    与えられた点の集合Pから集合Pの凸包体を構成する点を選択する。

- CGL_4_B  DiameterofaConvexPolygon  
    凸多角形gの直径(最遠頂点間距離)を求める。

- CGL_4_C  ConvexCut  
    凸多角形を直線(p1,p2)で切断したときに左側にできる凸多角形の面積を求める。

- CGL_5_A  ClosestPair  
    n個の点が作るペアの中で最も距離が近いペアを見つける。
...
- CGL_7_A  Intersection  
    二つの円c1,c2について、それらが
    1:離れているか、2:外接するか、3:交差するか、4:内接するか、5:一方がもう一方を内包するか
    判定する。

- CGL_7_D  CrossPointsofaCircleandaLine  円と直線の交点  
    半径rの円(cx,cy)と、q個の直線(p1<sub>i</sub>,p2<sub>i</sub>)との交点をそれぞれ求める。
